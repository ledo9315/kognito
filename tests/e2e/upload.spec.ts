...
test('uploading a pdf stores a readable source', async ({ page }) => {
  ...
  await page.getByLabel('Datei auswählen').setInputFiles('tests/sources/fixtures/sample.pdf')
...
test('a pdf without a text layer is refused with a reason', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Hinzufügen', exact: true }).click()
  await page.getByLabel('Datei auswählen').setInputFiles('tests/sources/fixtures/scanned.pdf')
  await page.getByRole('button', { name: 'Hochladen' }).click()
