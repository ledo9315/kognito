...
async function fixture(name: string, type: string) {
  const bytes = await readFile(`lib/fixtures/${name}`)
...
describe('files', () => {
  ...
  it('falls back to the file extension when the type is missing', async () => {
    const bytes = await readFile('lib/fixtures/sample.pdf')
...
const html = await readFile('lib/fixtures/article.html', 'utf8')
