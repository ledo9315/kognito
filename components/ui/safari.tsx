...
export interface SafariProps extends HTMLAttributes<HTMLDivElement> {
  ...
  mode?: SafariMode
  /**
   * Fills the screen around a picture whose ratio differs from the window's
   * 12:7. Pick the picture's own edge colour and the gap reads as margin.
   */
  screenBackground?: string
  /** Passed to next/image for the picture: preload it when it is the hero. */
  priority?: boolean
  /** Passed to next/image for the picture, the window's rendered width. */
  imageSizes?: string
  /** Passed to next/image; screenshots full of small text want more than 75. */
  imageQuality?: number
}
  ...
  return (
    ...
    >
      ...
      {!hasVideo && imageSrc && (
        ...
        >
          {/* Contain, not cover: a dashboard loses its side panels when it
              is cropped to the window's ratio. */}
          <Image
            src={imageSrc}
            alt=""
            fill
